...
import { QueryWithHelpers, HydratedDocument,Schema } from "mongoose";
import User from "./../../../database/schemas/User";

export enum ApiPostUserAction {
    View = "user",
    Moderate = "admin",
    Create = "provider",
}
...
class ApiPost {
    public req: Request;
    public res: Response;
    private user: QueryWithHelpers<typeof User, typeof User> extends typeof Schema(typeof User);
    ...
    public async fetchUser() {
        this.user = await users.findOne({
            accountToken: this.req.headers.authorization,
        });
    }
    ...
    public async create() {
        ...
        if (!this.hasPermissionsFor(ApiPostUserAction.Create)) return;
    }

    private hasPermissionsFor(action: ApiPostUserAction) {
        if (!this.user) return false;

        if (this.user.permissions.includes("admin")) return true;

        return this.user.permissions.includes(action.toString());
    }
}
...
export default async (req: Request, res: Response) => {
    ...
    function sanitizeTitle() {
        ...
        if (!title || typeof title != "string" || title.trim() == "")
            return req.file?.originalname.split(".").slice(0, -1).join(".");
